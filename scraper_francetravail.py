...
def parse_offer_summary(data: dict) -> tuple[str, str, str, str, Optional[str], Optional[str]]:
...
def search_offers(token: str, limiter: RateLimiter, keyword: str,
                  seen_ids: Set[str], max_results: Optional[int] = None) -> List[tuple]:
    ...
    while True:
        ...
        limiter.wait()
        end = min(start + batch - 1, max_results - 1) if max_results else start + batch - 1
        ...
        try:
            ...
            print(f"[SEARCH] Range {start}-{end} (Total: {content_range}): {len(results)} offers, {batch_new} new")
...
def get_offer_detail(token: str, offer_id: str, limiter: RateLimiter) -> Optional[JobOffer]:
...
def scrape_to_mongo(token: str, summaries: List[tuple], limiter: RateLimiter, 
                   collection, workers: int = 8, verify: bool = False):
        ...
        for i, future in enumerate(as_completed(futures), 1):
            try:
                ...
                if i % 10 == 0 or i == len(summaries):
                    ...
                    print(f"\r[SCRAPE] {pct:.0f}% | {i}/{len(summaries)} | "
