...
def extract_relevant_html(html_content: str) -> str:
    ...
    main_content = (
        ...
        soup.find('div', class_=re.compile(r'content|main|body|offer|offre', re.I)) or
        soup.find('div', id=re.compile(r'content|main|body|offer|offre', re.I))
    )
...
def extract_with_llm(html_content: str, url: str, keyword: str, 
                     limiter: RateLimiter) -> tuple[Optional[ServicePublicJobOffer], Optional[str]]:
    """
...
def transform_to_harmonized_schema(llm_data: dict, url: str, keyword: str) -> ServicePublicJobOffer:
    """
    ...
    date_limite_iso = convert_date_to_iso(date_limite) if isinstance(date_limite, str) else ''
    ...
    return ServicePublicJobOffer(
        ...
        entreprise=llm_data.get('collectivite') if llm_data.get('collectivite') else None,
        ...
        departement=llm_data.get('departement') if llm_data.get('departement') else None,
        date_limite=date_limite_iso,
        avantages=llm_data.get('avantages') if llm_data.get('avantages') else None,
        teletravail=llm_data.get('teletravail') if llm_data.get('teletravail') else None,
...
def scrape_keyword_parallel(keyword: str, collection, workers: int = 4, max_jobs: Optional[int] = None):
    """
    ...
    def process_one_job(job_url: str) -> Optional[ServicePublicJobOffer]:
        ...
        try:
            ...
            print(f"    [EXCEPTION in process_one_job] {type(e).__name__}: {str(e)[:100]}")
            return None
        finally:
            ...
            try:
                ...
                with lock:
                    ...
                    if i % 10 == 0 or i == len(all_job_urls):
                        ...
                        print(f"\r[PROGRESS] {pct:.0f}% | {i}/{len(all_job_urls)} | "
    ...
    print(f"Success: {stats['success']} ({stats['success']/len(all_job_urls)*100:.1f}%)")
