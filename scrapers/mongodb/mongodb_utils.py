...
Version mise à jour avec support de reference_apec pour APEC
"""
...
def bulk_upsert(collection, documents: List[Dict[str, Any]], id_field: str = "id") -> int:
    ...
    for doc in documents:
        # APEC SPECIAL: Si on utilise reference_apec, copier aussi dans 'id'
        if id_field == "reference_apec" and "reference_apec" in doc:
            doc["id"] = doc["reference_apec"]
    ...
    if skipped > 0:
        print(f"[DB WARNING] Skipped {skipped} documents missing '{id_field}' field")
...
# =============================================================================
# APEC-SPECIFIC HELPERS
# =============================================================================

def prepare_apec_documents(documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Préparer les documents APEC en copiant reference_apec vers id
    
    Args:
        documents: Liste de documents APEC bruts
    
    Returns:
        Liste de documents préparés avec 'id' = 'reference_apec'
    """
    prepared = []
    missing_ref = 0
    ...
    if missing_ref > 0:
        print(f"[DB WARNING] {missing_ref} documents without 'reference_apec' field")
